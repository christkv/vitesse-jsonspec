...
var mapFields = function(self, parent, field, x) {
  ...
  if(x.patternProperties) {
    validation = validation || new ObjectNode(parent, field, {});
    var patternValidation = new ObjectNode.Pattern()
    
    for(var pattern in x.patternProperties) {
      patternValidation.add(pattern, mapFields(self, parent, field, x.patternProperties[pattern]));
    }

    validation.addSpecialValidator(patternValidation);
  }
